...
func GetLobbyByInviteCode(c *fiber.Ctx, lobbyInviteID string, user models.User) (*utils.Lobby, error) {
	inviteKey := "invite:" + lobbyInviteID

	// Step 1: Get the lobby ID
	lobbyID, err := database.Redis.Get(c.Context(), inviteKey).Result()
	if err != nil {
		log.Println("redis get error (invite):", err)
